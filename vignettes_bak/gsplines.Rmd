...
unemp <- within(
  unemp,
  {
    year <- (as.numeric(date) - as.numeric(as.Date('2000-01-01')))/365.25
    month <- as.numeric(format(date, '%m'))
    
  })
summary(unemp)
sp3 <- gspline(quantile(unemp$year, 1:3/4), 3, 2)    
sp3
fit3 <- lm(unemployment ~ sp3(year), unemp)
unemp$fit3 <- predict(fit3)
xyplot(unemployment ~ date, unemp) + layer(panel.lines(x, unemp$fit3))
...
head(unemp)
sp <- gspline(as.Date('2008-12-01'), 2, -1)
```

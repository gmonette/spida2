...
pp <- xyplot(unemployment ~ date, unemp, type = 'b',
  ...
  layer(panel.lines(x, unemp$fit3d, col = 'blue', lty = 3)) +
  layer(panel.lines(x, unemp$fit2d, col = 'red', lty = 3)) 
pp
...
wald(fitper3)
pred_deriv <- expand.grid(month = seq(0,24, .05), year = 0, D = 1:3)
dim(pred_deriv)
head(pred_deriv)
LD <- with(pred_deriv, per3(month, D = D))
LD2 <- per3(pred_deriv$month, D = pred_deriv$D)
LD3 <- per3(pred_deriv$month, D = 1)
...
We construct a linear estimation matrix for the derivatives of the spline by
setting the coefficients of the secular portion to 0 and using the spline function
to generate coefficients to estimate various derivatives.
```{r}
wald(fitper3)
pred_deriv <- expand.grid(month = seq(0,24, .05), year = 0, D = 1:3)
dim(pred_deriv)
...
ww <- as.data.frame(wald(fitper3, LD))
pred3 <- expand.grid(month = pred$month, year = 0, D = 1:3)
pred3$labs <- reorder(factor(c('first','second','third')[pred3$D]), pred3$D) 

ww <- cbind(ww, pred3)
xyplot(coef ~ month | labs, ww, as.table = TRUE, 
       xlim = c(0,24),
       ylab = 'derivative',
       scales = list(x = list(at = 3* 0:8)),
       layout = c(1,3), type = 'l') +

...
#' paik_old(verdict ~ d.race + v.race, 
#'    counts = death.penalty$count,
#'    data = death.penalty, 
#'    leg.title = "Victims' race", xlab = "Defendants' race", 
#'    ylab = "Proportion receiving death penalty")
#' par(mar=c(4,2,0,2))
#' paik_old(verdict ~ v.race + d.race, counts = death.penalty$count, data = death.penalty, 
#'      xlab = "Victims' race", leg.title = "Defendants' race",leg.loc="topleft", 
#'      ylab = "", yaxt = "n")
#' paik_old(am ~ gear + carb, mtcars)
#' par(op)
#' @export 
paik_old <- function (formula, data, counts, resp.lvl = 2,  circle.mult = 1, 
...
#' New version of Paik-Agresti diagram
#' 
#' New version
#' 
...
paik <- function(form, data, ylab, xlab = xn, cex.factor = 10, ...){
  ...
  xyplot(y ~ x, dcond, groups = z, type = 'l', lwd = 2,
         xlab = xlab, ylab = ylab, ...,

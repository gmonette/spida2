...
#' 
#' The box.width argument to the default panel.barchart panel function
#' does not set the height/width of bars correctly. panel.barchart2
#' corrects this so the box.width argument can be a vector with
#' values for individual bars.
#' 
#' Except for the box.width argument, see the definition of parameters
#' in panel.barchart.
#' 
#' @export
panel.barchart2 <-
    ...
    if (horizontal) {
        if (is.null(groups)) {
            ...
            panel.rect(x = rep(origin, length(y)), y = y, height = rep(height, 
                length.out = length(y)), width = x - origin, border = border, 
        ...
        if (is.null(groups)) {
            ...
            panel.rect(x = x, y = rep(origin, length(x)), col = col, 
                border = border, lty = lty, lwd = lwd, width = rep(width, 
                  length.out = length(x)), height = y - origin, just = c("centre", 
